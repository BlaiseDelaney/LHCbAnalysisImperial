...
fin = TFile("/vols/lhcb/bd1316/B2ppKgamma/TMVA/JointSamples/B2pppigammaMC_truthmatched.root")#W/O cuts applied so B_plus_MM between 5000 and 5000 MeV and (pplus+p~minus).M() not in range of J/psi mass +/- 100 MeV
tin=fin.Get("DecayTree")
tin.GetEntries()

BDT = array.array('f',[0])

fout = TFile('/vols/lhcb/bd1316/B2ppKgamma/TMVA/FINALcutsANA/B2pppigammaMC_sig.root',"RECREATE")
tout = tin.CopyTree('0')
...
for n in range(N):
    ...
    Vars['logKplus_IPCHI2_OWNPV'][0] = log(getattr(tin, 'piplus_IPCHI2_OWNPV'))
