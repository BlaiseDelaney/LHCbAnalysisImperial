...
path_destination = "/vols/lhcb/bd1316/B2ppKgamma/B2ppKgamma/"

fname = "B2pphgamma_S21_protonID" #complete TTree
fin = TFile(path_origin+fname+".root")
tin2 = fin.Get("B2pphgamma_Tuple/DecayTree")

fout = TFile(path_destination+fname+"_Lambda_c_BkdEnhancedPKPI","RECREATE")
fout.cd()
...
h_pKpi_mass.Draw()
h_pKpi_mass.SetTitle("#Lambda_{c}#rightarrowK^{+}#bar{p}#pi^{-}")
h_pKpi_mass.GetXaxis().SetTitle("M_{K^{+}#bar{p}#pi^{-}} [MeV]")


out_path_pdf = "/home/hep/bd1316/public_html/Temp/"
cv.SaveAs(out_path_pdf+"Lambda_c_KpPi_enhancedBKD_Bbadlyvertexed.pdf")
